 ...
 * Text Domain: wp-dependency-minification
 * License: GPLv2+
...
class Dependency_Minification {
	...
	static function admin_menu() {
		self::$admin_page_hook = add_submenu_page(
			...
			__( 'Dependency Minification', 'wp-dependency-minification' ),
			__( 'Dep. Minification', 'wp-dependency-minification' ),
		...
		if ( empty( $GLOBALS['wp_rewrite']->permalink_structure ) ) {
			...
			<div class="error">
				...
				echo sprintf(
					...
					__( 'Dependency Minification', 'wp-dependency-minification' ),
					sprintf(
						__( 'Pretty permalinks are not enabled in your %1$s, which is required for this plugin to operate. Select something other than Default (e.g. ?p=123)', 'wp-dependency-minification' ),
						sprintf(
							'<a href="%1$s">%2$s</a>',
							admin_url( 'options-permalink.php' ),
							__( 'Permalinks Settings', 'wp-dependency-minification' )
						)
					)
				);
				?></p>
		...
		<div class="updated">
			<?php if ( 'expire' === $updated_task ) : ?>
				...
				echo esc_html( sprintf(
					_n( 'Expired %d minified dependency.', 'Expired %d minified dependencies.', $updated_count, 'wp-dependency-minification' ),
					$updated_count
				));
				?></p>
			<?php elseif ( 'purge' === $updated_task ) : ?>
				<p><?php
				echo esc_html( sprintf(
					_n( 'Purged %d minified dependency.', 'Purged %d minified dependencies.', $updated_count, 'wp-dependency-minification' ),
					$updated_count
				) );
				?></p>
			<?php else: ?>
				<p><?php esc_html_e( 'Updated.', 'wp-dependency-minification' ) ?></p>
			<?php endif; ?>
		</div>
		<?php
	}
	...
	static function admin_ajax_handler() {
		if ( ! current_user_can( self::$options['admin_page_capability'] ) ) {
			wp_die( __( 'You are not allowed to do that.', 'wp-dependency-minification' ) );
		}
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], self::AJAX_ACTION ) ) {
			wp_die( __( 'Nonce check failed. Try reloading the previous page.', 'wp-dependency-minification' ) );
		}
		$updated_count = 0;
	...
	static function admin_plugin_action_links( $links, $file ) {
		if ( plugin_basename( __FILE__ ) === $file ) {
			...
			$admin_page_link = sprintf( '<a href="%s">%s</a>', esc_url( $admin_page_url ), esc_html__( 'Settings', 'wp-dependency-minification' ) );
	...
	static function admin_page() {
		if ( ! current_user_can( self::$options['admin_page_capability'] ) ) {
			wp_die( __( 'You cannot access this page.', 'wp-dependency-minification' ) );
		}
		$nonce = wp_create_nonce( self::AJAX_ACTION );
		?>
		<div class="wrap">
			<div class="icon32" id="icon-tools"><br></div>
			<h2><?php esc_html_e( 'Dependency Minification', 'wp-dependency-minification' ) ?></h2>
			...
			<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ) ?>" method="post">
				...
				<?php if ( self::$options['disable_if_wp_debug'] && ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ): ?>
					<div class="error">
						<p><?php esc_html_e( 'Dependency Minification is disabled. Any minified dependencies below are cached. To minify new dependencies, disable WP_DEBUG or filter disable_if_wp_debug to be false.', 'wp-dependency-minification' ); ?></p>
					</div>
				<?php endif; ?>

				<?php if ( empty( $minified_dependencies ) ) : ?>
					<p>
						<em><?php esc_html_e( 'There are no minified dependencies yet. Try browsing the site.', 'wp-dependency-minification' ); ?></em>
					</p>
				<?php else : ?>
					<div class="tablenav top">
						<div class="alignleft actions">
							<select name="depmin_task">
								...
								<option value="expire"><?php esc_html_e( 'Expire', 'wp-dependency-minification' ) ?></option>
								<option value="purge"><?php esc_html_e( 'Purge', 'wp-dependency-minification' ) ?></option>
							</select>
						...
						<?php foreach ( array( 'thead', 'tfoot' ) as $i => $tcontainer ) : ?>
							<<?php echo $tcontainer; // xss ok ?>>
								<tr>
									...
									<th scope="col" class="manage-column column-dependencies"><?php esc_html_e( 'Dependencies', 'wp-dependency-minification' ) ?></th>
									<th scope="col" class="manage-column column-count"><?php esc_html_e( 'Count', 'wp-dependency-minification' ) ?></th>
									<th scope="col" class="manage-column column-compression"><?php esc_html_e( 'Compression', 'wp-dependency-minification' ) ?></th>
									<th scope="col" class="manage-column column-type"><?php esc_html_e( 'Type', 'wp-dependency-minification' ) ?></th>
									<th scope="col" class="manage-column column-last-modified"><?php esc_html_e( 'Last Modified', 'wp-dependency-minification' ) ?></th>
									<th scope="col" class="manage-column column-expires"><?php esc_html_e( 'Expires', 'wp-dependency-minification' ) ?></th>
								</tr>
							</<?php echo $tcontainer;  // xss ok ?>>
						<?php endforeach; ?>
							...
							<?php foreach ( $minified_dependencies as $option_name => $minified_dependency ) : ?>
								...
								<tr id="<?php echo esc_attr($option_name) ?>" valign="top">
									<th scope="row" class="check-column">
										<label class="screen-reader-text" for="cb-select-<?php echo esc_attr($option_name) ?>"><?php esc_html_e( 'Select minified dependency', 'wp-dependency-minification' ) ?></label>
									...
									<td class="column-dependencies">
										<strong>
											<?php for ( $i = 0; $i < count( $deps ); $i += 1 ) : ?>
												<a href="<?php echo esc_url( $deps[$i]['src'] ) ?>" target="_blank" title="<?php esc_attr_e( 'View unminified source (opens in new window)', 'wp-dependency-minification' ) ?>"><?php echo esc_html( $deps[$i]['handle'] ) ?></a><?php if ( $i + 1 < count( $handles ) ) { esc_html_e( ', ' ); } ?>
											<?php endfor; ?>
										</strong>
										<?php if ( ! empty( $error ) ) : ?>
											<div class="error">
												<p>
													...
													<span class="trash">
														...
														?>
														<a class="submitdelete" title="<?php esc_attr_e( 'Delete the cached error to try again.', 'wp-dependency-minification' ) ?>" href="<?php echo esc_url( add_query_arg( $_link_params, admin_url( 'admin-ajax.php' ) ) ) ?>">
															<?php esc_html_e( 'Try again', 'wp-dependency-minification' ) ?>
														</a>
													</span>
												</p>
											</div>
											...
											<div class="row-actions">
												<?php if ( empty( $error ) ) : ?>
													<span class="expire">
														...
														?>
														<a href="<?php echo esc_url( add_query_arg( $_link_params, admin_url( 'admin-ajax.php' ) ) ) ?>" title="<?php esc_attr_e( 'Expire this item to gracefully regenerate', 'wp-dependency-minification' ) ?>"><?php esc_html_e( 'Expire', 'wp-dependency-minification' ) ?></a> |
													</span>
													<span class="trash">
														<?php
														$_link_params = $link_params;
														$_link_params['depmin_task'] = 'purge';
														?>
														<a class="submitdelete" title="<?php esc_attr_e( 'Purge item from cache (delete immediately; NOT recommended)', 'wp-dependency-minification' ) ?>" href="<?php echo esc_url( add_query_arg( $_link_params, admin_url( 'admin-ajax.php' ) ) ) ?>"><?php esc_html_e( 'Purge', 'wp-dependency-minification' ) ?></a> |
													</span>
													<span class="view">
														<a href="<?php echo esc_url( $minified_src ) ?>" target="_blank" title="<?php esc_attr_e( 'View minified dependencies (opens in new window)', 'wp-dependency-minification' ) ?>" rel="permalink"><?php esc_html_e( 'View minified', 'wp-dependency-minification' ) ?></a>
													</span>
												<?php else: ?>
													<span class="trash">
														<?php
														...
														$_link_params['depmin_task'] = 'purge';
														?>
														<a class="submitdelete" title="<?php esc_attr_e( 'Delete the cached error to try again.', 'wp-dependency-minification' ) ?>" href="<?php echo esc_url( add_query_arg( $_link_params, admin_url( 'admin-ajax.php' ) ) ) ?>"><?php esc_html_e( 'Try again', 'wp-dependency-minification' ) ?></a>
													</span>
												<?php endif; ?>
											</div>
										<?php endif; ?>
									...
									<td class="count column-compression"><?php
										if ( empty( $unminified_size ) ) {
											esc_html_e( 'N/A', 'wp-dependency-minification' );
										} else {
											$min = strlen( $contents );
											$max = $unminified_size;
											$percentage = round( 100 - ( $min / $max ) * 100 );
											printf( '<meter min=0 max=100 value="%d" title="%s">', $percentage, esc_attr( sprintf( __( '(%1$d / %2$d)', 'wp-dependency-minification' ), $min, $max ) ) );
											print esc_html( sprintf( __( '%1$d%%', 'wp-dependency-minification' ), $percentage ) );
											print '</meter>';
										}
									...
									<?php foreach ( $date_cols as $col => $time ) : ?>
										<td class="<?php echo esc_attr( $col ) ?> column-<?php echo esc_attr( $col ) ?>">
											<?php if ( $pending ) : ?>
												<em><?php esc_html_e( '(Pending)', 'wp-dependency-minification' ) ?></em>
											<?php else: ?>
